...
class Command(BaseCommand):
    ...
    def _data_crate(self):
        ...
        obj, created = Avarage.objects.update_or_create(name='taurine', defaults={'avarage': truncate(query['taurine'])})

        query = Dry.objects.filter(calcium__gt=0).aggregate(calcium=Avg('calcium'))
        obj, created = Avarage.objects.update_or_create(name='calcium', defaults={'avarage': truncate(query['calcium'])})

        query = Dry.objects.filter(phosphorus__gt=0).aggregate(phosphorus=Avg('phosphorus'))
        obj, created = Avarage.objects.update_or_create(name='phosphorus', defaults={'avarage': truncate(query['phosphorus'])})

        query = Dry.objects.filter(magnesium__gt=0).aggregate(magnesium=Avg('magnesium'))
        obj, created = Avarage.objects.update_or_create(name='magnesium', defaults={'avarage': truncate(query['magnesium'])})

        query = Dry.objects.filter(magnesium__gt=0).aggregate(magnesium=Avg('magnesium'))
        obj, created = Avarage.objects.update_or_create(name='magnesium', defaults={'avarage': truncate(query['magnesium'])})

        query = Dry.objects.filter(epa__gt=0).aggregate(epa=Avg('epa'))
        obj, created = Avarage.objects.update_or_create(name='epa', defaults={'avarage': truncate(query['epa'])})

        query = Dry.objects.filter(dha__gt=0).aggregate(dha=Avg('dha'))
        obj, created = Avarage.objects.update_or_create(name='dha', defaults={'avarage': truncate(query['dha'])})

        query = Dry.objects.filter(omega3__gt=0).aggregate(omega3=Avg('omega3'))
        obj, created = Avarage.objects.update_or_create(name='omega3', defaults={'avarage': truncate(query['omega3'])})

        query = Dry.objects.filter(omega6__gt=0).aggregate(omega6=Avg('omega6'))
        obj, created = Avarage.objects.update_or_create(name='omega6', defaults={'avarage': truncate(query['omega6'])})

        query = Guaranteed.objects.filter(moisture__gt=0).aggregate(moisture=Avg('moisture'))
        obj, created = Avarage.objects.update_or_create(name='moisture', defaults={'avarage': truncate(query['moisture'])})

        query = Calorie.objects.aggregate(protein=Avg('protein'), fat=Avg('fat'), carbs=Avg('carbs'), total=Avg('total'),)

        obj, created = Avarage.objects.update_or_create(name='calorie_protein', defaults={'avarage': truncate(query['protein'])})
        obj, created = Avarage.objects.update_or_create(name='calorie_fat', defaults={'avarage': truncate(query['fat'])})
        obj, created = Avarage.objects.update_or_create(name='calorie_carbs', defaults={'avarage': truncate(query['carbs'])})
        obj, created = Avarage.objects.update_or_create(name='calorie_total', defaults={'avarage': truncate(query['carbs'])})
